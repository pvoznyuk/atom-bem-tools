...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'bem-tools:run': () => this.run(),
      'bem-tools:to-json': () => this.run({to: BEMJSON}),
      'bem-tools:to-css': () => this.run({to: CSS}),
      'bem-tools:to-scss': () => this.run({to: SCSS}),
    }));
  },
  ...
  run(_config = {}) {
    const config = Object.assign({}, atom.config.get('bem-tools'), _config);
  ...
  _jsonToHTML(string, config) {
    ...
    if (typeof structure === 'object') {
      const outputHtml = pretty(new BEMHtml({
        modificatorSeparator: config.modPrefix,
        addDefautTagAttributes: true
      }).toHtml(structure));
      if (outputHtml) {
        const textEditor = atom.workspace.getActiveTextEditor();
        const cssOutput = new BEMCss({
          modificatorSeparator: config.modPrefix,
          compileTo: 'scss',
          tab: config.scssTabs.replace('\\t', '\t')
        }).toCSS(structure);
